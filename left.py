...
f = open("output.txt", "w") # DONT CHANGE THIS ##### CREATE A TXT FILE OF THIS NAME IN THE FOLDER
f.close()
...
for values in content:
    try:
        ...
        pass
print("Reading left side points...")
time.sleep(2)
...
### INSERT SLICING VALUE WRT Y AXIS ###
    ...
    if(int(x_pts1[i]) > 200): # insert the slicing value here
        continue
...
while(z>0):
    ...
    if(c%2 == 0):
        ...
        if(len(top_arr_3) != 0):
            x_avg = sum(top_arr_3) // len(top_arr_3)
            # print(x_avg)
        ...
        for i in top_arr_sorted_1:
            ...
            if(x_pts[pos] > x_avg -10 and x_pts[pos] < x_avg + 10):
    ...
    # The points are sorted in descending order
        ...
        for i in top_arr_sorted_1:
            ...
            # lst.append(y_pts[pos])
            #lst.append(int(y_pts[pos])- lim_dist) ##################
            if(y_pts[pos] > x_avg -10 and y_pts[pos] < x_avg + 10):
                lst.append(y_pts[pos])
            else:
                lst.append(x_avg)
                lst.append(y_pts[pos])
...
with open('output.txt', 'a') as outfile:
    ...
    outfile.write("\n")
    outfile.write("####################################################################\n")
print("LEFT SIDE POINTS SORTED SUCCESSFULLY")
